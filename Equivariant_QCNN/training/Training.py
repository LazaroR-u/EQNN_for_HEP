...
from benchmarking.Benchmarking import accuracy_test
...
def circuit_training(X_train, Y_train, U, U_params, embedding_type, circuit, cost_fn, opt_type = "Adam", steps=50, initial_learning_rate=0.01, batch_size=128, decay_factor=0.5, decay_steps=10):
    if circuit == 'QCNN':
        if U == 'U_SU4_no_pooling' or U == 'U_SU4_1D' or U == 'U_9_1D' or U == "U2_equiv":
    ...
    for it in range(steps):
        ...
        predictions = [QCNN_circuit.QCNN(x, params, U, U_params, embedding_type, cost_fn) for x in X_train[batch_size:batch_size*5]]
